const translations = {
    ...
    'es': {
        ...
        'infoDesc': 'Soy un desarrollador de software altamente versátil y orientado a resultados. Mis estudios me han proporcionado una sólida base de experiencia en diversos aspectos tecnológicos y desafíos, abarcando una amplia gama de herramientas y lenguajes de programación, los cuales detallo en los siguientes apartados. Mi capacidad para adaptarme rápidamente a nuevas herramientas y metodologías, junto con mi pasión por la resolución de problemas y el enfrentamiento de nuevos retos, me ha permitido contribuir efectivamente a los proyectos en los que he participado. Con una actitud proactiva y una mentalidad orientada al aprendizaje continuo, estoy comprometido a mantenerme actualizado con las últimas tecnologías en el campo del desarrollo de software. He demostrado habilidades sólidas en el uso de herramientas de control de versiones como Git, facilitando la colaboración y la gestión de código en un entorno de desarrollo ágil. Esta experiencia ha sido fundamental para el seguimiento de metodologías ágiles, permitiéndome colaborar efectivamente con equipos multidisciplinarios en la planificación, ejecución y entrega de proyectos.',
        'exp': 'Experiencia laboral',
        'expList': [
            [
                'NTT Data',
                `En mi puesto en NTT Data, he obtenido experiencia valiosa trabajando con tecnologías como ASP, ASPX y SQL Server. He estado involucrado en el desarrollo y mantenimiento de aplicaciones web, utilizando ASP.NET y ASPX para crear soluciones funcionales. Además, he gestionado bases de datos con SQL Server, participando en la optimización de consultas y el diseño de esquemas.

                He utilizado Git para el control de versiones, lo que ha facilitado la colaboración efectiva en equipo y la gestión de código. Esta experiencia ha sido complementada con el seguimiento de metodologías ágiles, donde he trabajado en sprints y participado en reuniones diarias, lo que me ha permitido adaptarme a cambios rápidos y contribuir a la entrega exitosa de proyectos.

                Mi experiencia en esta empresa ha fortalecido mis habilidades para trabajar en equipo y manejar proyectos dentro de un entorno ágil, proporcionando una base sólida para mi desarrollo profesional continuo.`,
                'Febrero - Mayo 2024',
                'images/NTT-Data-Logo.png'],
            [
                'Inmobiliaria Costa Soñada',
                'En mi rol en Costa Soñada, trabajé de manera constante en equipo para desarrollar y mantener la página web de la empresa. Participé en la implementación de nuevas funcionalidades y en el mantenimiento general del sitio, utilizando JavaScript, HTML, CSS y WordPress. Mi experiencia incluyó colaborar estrechamente con diseñadores y desarrolladores para asegurar que las mejoras y ajustes se integraran de manera efectiva, optimizando la experiencia del usuario y la funcionalidad del sitio web.',
        ...
        'projectsList': [
            [
                ...
                'En mi proyecto de TFG, desarrollé un sistema de detección de caídas en personas de tercera edad utilizando una cámara y tecnologías de inteligencia artificial. El sistema emplea Python junto con TensorFlow y Keras para entrenar modelos de aprendizaje automático que identifican caídas en tiempo real. Utilicé OpenCV para el procesamiento de imágenes, permitiendo la captura y análisis continuo del video. El objetivo del proyecto es mejorar la seguridad de los adultos mayores, alertando rápidamente en caso de una caída para facilitar una intervención oportuna.',
                'Tecnologías usadas: Python, OpenCV, TensorFlow, Keras, MediaPipe.', '1', 'https://www.youtube.com/embed/cUkGU9l-Hvk?si=_YPxxvrOLJs-RxjV'
            ],
            [
...
/* <iframe width="560" height="315" src="https://www.youtube.com/embed/cUkGU9l-Hvk?si=_YPxxvrOLJs-RxjV" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe> */

const enMaster = 'University Master\'s degree in development of applications and web services'
const enGrado = 'Degree in Computer Engineering'

function changeLanguage(lang) {
    const selectedTranslations = translations[lang];

    const titleIntro = document.getElementById('titleIntro')
    titleIntro.innerText = selectedTranslations.info
    const titleExp = document.getElementById('expTitle')
    titleExp.innerText = selectedTranslations.exp
    const infoIntroduccion = document.getElementById('intro')
    infoIntroduccion.innerText = selectedTranslations.infoDesc
    const titleTecn = document.getElementById('tecnTitle')
    titleTecn.innerText = selectedTranslations.tecn
    const languages = document.getElementById('languages')
    languages.innerText = selectedTranslations.languages

    const expContainer = document.getElementById('expContent')
    expContainer.innerHTML = ''
    const expList = selectedTranslations.expList
    expList.forEach(card => {
        ...
        expContainer.appendChild(cardElement);
    });

    const academContainer = document.getElementById('academContainer')
    academContainer.innerHTML = ''
    const academList = selectedTranslations.academList
    academList.forEach(card => {
        // Crear el elemento de la tarjeta
        const cardElement = document.createElement('div');
        ...
        academContainer.appendChild(cardElement);
    });

    const proyectosContainter = document.getElementById('proyectosContainer')
    proyectosContainter.innerHTML = ''
    const proyectosList = selectedTranslations.projectsList
    proyectosList.forEach(element => {
        ...
        proyectosContainter.appendChild(divElement)
    })

    const tecnContainer = document.getElementById('tecnContainer')
    tecnContainer.innerHTML = ''
    const tecnLang = selectedTranslations.tecnLang
    const tecnLangList = selectedTranslations.tecnLangList
    const tecnLib = selectedTranslations.tecnLib
    const tecnLibList = selectedTranslations.tecnLibList
    const tecnDB = selectedTranslations.tecnDB
    const tecnDBList = selectedTranslations.tecnDBList
    const tecnTools = selectedTranslations.tecnTools
    const tecnToolsList = selectedTranslations.tecnToolsList

    tecnContainer.innerHTML = `
        <h4>${tecnLang}</h4>
        <p>${tecnLangList.join(' | ')}</p>
        <hr>
        <h4>${tecnLib}</h4>
        <p>${tecnLibList.join(' | ')}</p>
        <hr>
        <h4>${tecnDB}</h4>
        <p>${tecnDBList.join(' | ')}</p>
        <hr>
        <h4>${tecnTools}</h4>
        <p>${tecnToolsList.join(' | ')}</p>
        <hr>
    `
    ...
    const softSkillsList = selectedTranslations.softSkillsList
    var count = 0
    softSkillsList.forEach(element => {
